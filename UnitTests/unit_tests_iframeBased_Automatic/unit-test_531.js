describe('String N/A: no nav nodes', function() {
    it('no nav nodes should be equal to na', function() {
       ...
       // <variant1>
       var res = LevelAccess_AccessEngine.runTest_returnOutcome("531");
       expect(res).toBe('na');
       // </variant1>
       // <variant2>
       res = LevelAccess_AccessEngine.runTest_returnNumbers("531");
       expect(res).toBe("");       
       // </variant2>
       // <variant3>
       res = LevelAccess_AccessEngine.runTest_returnInstances("531");
...
describe('String N/A: two nav node with no accessible name', function() {
   it('two nav node with no accessible name should be equal to na', function() {
      document.querySelector("#testing").contentWindow.document.head.innerHTML = "";
      document.querySelector("#testing").contentWindow.document.body.innerHTML = "<nav>Test</nav><nav>Test2</nav>";
      LevelAccess_AccessEngine.setWindowUnderTest(document.querySelector("#testing").contentWindow);
      // <variant1>
      var res = LevelAccess_AccessEngine.runTest_returnOutcome("531");
      expect(res).toBe('na');
...
describe('String Fail: two nav nodes have an aria-label attribute set to the same text value', function() {
		it('Two nav nodes have an aria-label attribute set to the same text value should be equal to fail', function() {
			document.querySelector("#testing").contentWindow.document.head.innerHTML = "";
			document.querySelector("#testing").contentWindow.document.body.innerHTML = "<nav aria-label='describe'>Test</nav><nav aria-label='describe'>Mike</nav>";
