...
export function test325() {
	aeKernel.addAutomaticTest("325", {
		...
		fixType: "",
		testFunc: function() {
			const relevantCssSelector = 'ul[data-ae_vis][data-ae_avat]:not([data-ae_ar="null"]):not([data-ae_ar="list"])';
			return new AutoTestShell(
				relevantCssSelector,
				`${relevantCssSelector}:not([data-ae_ar="directory"]):not([data-ae_ar="group"]):not([data-ae_ar="listbox"]):not([data-ae_ar="menu"]):not([data-ae_ar="menubar"]):not([data-ae_ar="radiogroup"]):not([data-ae_ar="tablist"]):not([data-ae_ar="toolbar"]):not([data-ae_ar="tree"])`
			);
		}
	});
}
