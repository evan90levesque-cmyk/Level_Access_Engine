...
export function test109() {
	...
	aeKernel.addGuidedAutomaticTest("109", {
		bestPractice: 368,
		testId: 109,
		introduced: "0.9",
		mediaType: mediaTypes.WEB,
		description: "One or more elements, excluding elements with a size of 1px by 1px, not intentionally hidden in the DOM, contains text that is 18 point in size or larger; or bold text that is 14 point in size or larger has a luminosity contrast ratio below 3:1.",
		metaText: "This element contains text that is 18 point in size or larger; or bold text that is 14 point in size or larger that has a luminosity contrast ratio below 3:1",
		metaTextDetail: "This {{tag-name}} contains text with a background color of {{data-ae_bgcolor}} and foreground color of {{data-ae_color}} that is 18 point in size or larger; or bold text that is 14 point in size or larger that has a luminosity contrast ratio of {{data-ae_cr}}, which is below 3:1",
		fixType: "",
		testFunc: function() {
			const relevantCssSelector = '*:not(body):not([data-ae_tiny])[data-ae_vis]:not(script):not(noscript):not(title)[data-ae_ts="3"]';
			const testShell = new AutoTestShell(
				relevantCssSelector
			);
			testShell.set_candidateSetNodesFunc(function() {
				return testUtil_colorContrast(aeKernel.rootTestNode, relevantCssSelector, 3);
			});
			return testShell;
		}
	});
