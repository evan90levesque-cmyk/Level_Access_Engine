 ...
 */
export function test720() {
	aeKernel.addStage1MarkdownFunc("getBefore_FromStyles", getBefore_FromStyles);
	aeKernel.addStage1MarkdownFunc("getAfter_FromStyles", getAfter_FromStyles);
	aeKernel.addStage4MarkdownFunc("calcAccNamesForCertainElements", calcAccNamesForCertainElements);

	aeKernel.addGuidedAutomaticTest("720", {
		bestPractice: 338,
		testId: 720,
		introduced: "2.15",
		mediaType: mediaTypes.WEB,
		description: "One or more select elements, without an aria-assigned role, not intentionally hidden in the DOM and available to assistive technologies, has a suspicious accessible name value (i.e. under 4 characters long).",
		metaText: "This select element has a suspicious accessible name",
		metaTextDetail: "This select element has a suspicious accessible name value of: {{data-ae_an}}",
		fixType: "",
		testFunc: function() {
			const relevantCssSelector = 'select[data-ae_ar="null"][data-ae_vis][data-ae_avat][data-ae_an]';
