import {testTypes, origPositionAttsFromStyle} from "../../Utils/aeUtils.js";

/**
* Returns opening xml tag only of instance with original attributes + values restored
* @returns {String}
*/
export function origiOSXmlFromHTML(node) {
	...
	var result = origPositionAttsFromStyle(node.getAttribute("style"));
	origXml = origXml +
		'width="' + result.width.replace(/"/g, '\\"') +
		'" height="' + result.height.replace(/"/g, '\\"') +
		'" x="' + result.x.replace(/"/g, '\\"') +
		'" y="' + result.y.replace(/"/g, '\\"') + '" ';
 ...
 * @returns {{}}
 */
export function formatOutcomeForAccessContinuumMobileiOS(instance, test, type) {
	var outcome  = {};
	...
	if (instance.hasAttribute("data-xmlnodetype")) {
		element = origiOSXmlFromHTML(instance).substring(0, 2998);  // data-base takes string to 3000
	} else {
		element = null;
	}
	...
	if (note !== undefined) {
		if (instance.hasAttribute("data-xmlnodetype")) {
			attributeDetail = note.replace("[[object]]", instance.getAttribute("data-xmlnodetype").split("XCUIElementType")[1]);
		} else {
