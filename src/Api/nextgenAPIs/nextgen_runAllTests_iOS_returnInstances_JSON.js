...
import { formatNextGenResultForiOS } from "./formatNextGenResultForiOS";
import { resultsAsJSON } from "../ApiUtils/resultsAsJSON";
import { logger } from "../../Utils/Logger";
export function nextgen_runAllTests_iOS_returnInstances_JSON(testTypeArray = [testTypes.AUTOMATIC]) {
...
function buildAssertion(test, testId, testTypeName, resultsMetadata) {
	...
	for (const failingNode of resultsMetadata.failingNodes) {
		try {
			results.push(formatNextGenResultForiOS(failingNode, test));
		} catch (err) {
			logger.error(err);
		}
	}
